...
def generate_medusa_buffers(medusa_choices, device="cuda"):
    ...
    retrieve_indices = jt.concat([jt.zeros((retrieve_indices.shape[0], 1), dtype=jt.int64), retrieve_indices], dim=1)
...
def get_nucleus_one_token(logit, temperature, top_p):
    ...
    if top_p >= 1:
        return jt.multinomial(jt.nn.softmax(logit / temperature, dim=-1), 1)
    logit = logit / temperature
    ...
    indices_to_remove = sorted_indices_to_remove.scatter(dim=1, index=sorted_indices, src=sorted_indices_to_remove)
    logit[indices_to_remove] = float('-inf')
    sampled_tokens = jt.multinomial(jt.nn.softmax(logit, dim=-1), 1)
    return sampled_tokens
...
def generate_candidates(medusa_logits, logits, tree_indices, retrieve_indices, temperature = 0, posterior_threshold=0.3, posterior_alpha = 0.09, top_p=0.8, sampling = 'typical', fast = False):
    ...
    candidates_medusa_logits = jt.topk(medusa_logits[:, 0, -1], TOPK, dim = -1).indices
    ...
    tree_candidates_ext = jt.concat([tree_candidates, jt.zeros((1), dtype=jt.int64, device=tree_candidates.device)], dim=0)
...
def get_nucleus_posterior_mask(logits, candidates, temperature, top_p):
    ...
    if top_p >= 1:
        sampled_tokens = jt.multinomial(jt.nn.softmax(logits, dim=-1), 1)
...
def evaluate_posterior(
    ...
    if temperature == 0:
        ...
        if accept_length == 0:
            ...
            best_candidate = jt.argmax(candidates_accept_length).astype(jt.int64)
